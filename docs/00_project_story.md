...
phases), and an **offset** (the cycle's shift relative to *t = 0*). The decision
...
black-box** optimization problem, which is the case metaheuristics are built for.

## What we threw at it

We tried most of the standard methods, which is why this library exists:
...
- A **Navier–Stokes fluid analogy** for green-time allocation (treat traffic as a
  compressible flow). An interesting idea that did not win.
...
of a percent of optimal. There was almost no congestion for signal timing to
relieve. The whole optimizer tournament was, for *that* instance, overkill.

The lesson was a meta-lesson: understand your problem before you optimize it. We
spent days tuning swarms when an afternoon of analyzing the demand would have told
us the ceiling was nearly flat.
...
2. **The understanding**: the theory of how each method balances exploration vs.
   exploitation, captured in the [interactive explainer](../web/index.html) and in
   [`01_algorithms.md`](01_algorithms.md).

That's this repository: a reusable package plus a teaching site, both grounded in a
real (if anticlimactic) optimization war story.
